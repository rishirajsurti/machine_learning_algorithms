    ...
    def backPropagate(self, targets, N):
        ...
        for j in range(self.n_hidden):
            for k in range(self.n_output):
                ...
                self.wo[j][k] = self.wo[j][k] + N*change + 2*gamma*self.wo[j][k];
                 
        # update input weights
        for i in range(self.n_input):
            for j in range(self.n_hidden):
                change = hidden_deltas[j]*self.ai[i]
                self.wi[i][j] = self.wi[i][j] + N*change + 2*gamma*self.wi[i][j];
        ...
        for i in range(self.n_input):
            for j in range(self.n_hidden):
                error_beta += 0.01*self.wi[i][j];

        for j in range(self.n_hidden):
            for k in range(self.n_output):
                error_alpha += 0.01*self.wo[j][k];

        for k in range(len(targets)):
             error += 0.5*(((targets[k]-self.ao[k])**2 ));
        return error
        #error += 0.5*(((targets[k]-self.ao[k])**2 ) + error_alpha + error_beta) ;
...
n.train(train_data_features_n, train_data_target_i, 100, 0.5);
